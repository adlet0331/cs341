    ...
    visit(
      overloaded{
        ...
        [&](socket_data::SysSentStatus currSysSentsock) {
          ...
          if(currPacketType == PACKET_TYPE_SYNACK && 
          ((currSysSentsock.myip == INADDR_ANY && currSysSentsock.myport == destination_port) || 
          (currSysSentsock.myip == destination_ip && currSysSentsock.myport == destination_port))
          ){
        ...
        [&](socket_data::SynRcvdStatus currSynRcvdsock) {
          ...
          currPacketType == PACKET_TYPE_ACK){
        ...
        [&](socket_data::EstabStatus& currEstabsock) {
            ...
            if (currPacketType == PACKET_TYPE_FINISH){
              ...
              SocketReceiveBufferMap.count(make_pair(socketfd,processid)) != 0 &&
            ...
            // Write Send 한 패킷
              ...
              //버퍼가 없을때(이전에 write 패킷을 보낸 적이 없을 때 하나 만들어주기)
              if (SocketReceiveBufferMap.count(make_pair(socketfd, processid)) == (size_t)0) {
                ...
                SocketReceiveBufferMap[make_pair(socketfd, processid)] = make_pair(receiveBuffer, 0);
              }
              ...
              if (currEstabsock.ACK + 1000000 < SEQNum + 1000000) return;
                ...
                currEstabsock.ACK = SEQNum + datasize;
            ...
            // Write Send 한 후 돌아온 ACK 패킷 
              ...
              if (!send_queue.empty()){
                ...
                if (queuedSEQ + frontsize == ACKNum){
                  send_queue.pop_front();
                  this->returnSystemCallCustom(frontPacket.syscallUUID, frontsize);
...
void TCPAssignment::received_unreliable_packet(int sockfd, int pid, MyPacket receivedPacket){
  ...
  while(it != await_packet_list.end()){
    ...
    if (awaitseqNum + 1 == receivedackNum){
      ...
      printf("HANDSHAKE PACKET WELL RECEIVED %lu\n", await_packet_list.size());
      return;
    }
    else{
      it++;
...
void TCPAssignment::timerCallback(any payload) {
  ...
  if (isWriter){
    ...
    for(MyPacket myPacket:send_queue){
      ...
      uint32_t mySEQ = myPacket.SeqNum();

      //if (myPacket.isSent) UpdateTOI(timerStartedTime);
      if (mySEQ == revisedackNum){
